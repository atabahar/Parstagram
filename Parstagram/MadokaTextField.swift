...
//  Parstagram
//
//  Created by Ata Bahar on 6/2/19.
...
@IBDesignable open class MadokaTextField: TextFieldEffects {
    ...
    /**
     ...
     // Username text inside the textField
     */
    ...
    private func updateBorder() {
        ...
        path = UIBezierPath(roundedRect: CGRect(origin: CGPoint(x: 0, y: 0), size: CGSize(width: rect.width, height: rect.height)), cornerRadius: rect.width / 2)
        //path = UIBezierPath(roundedRect: CGRect(origin: CGPoint(x: 0, y: 0), size: CGSize(width: rect.width, height: rect.height)), cornerRadius: rect.width / 2)
//        path = UIBezierPath(roundedRect: CGRect(x: rect.origin.x + borderThickness, y: rect.height - borderThickness, width: rect.width, height: rect.height), cornerRadius: rect.width / 2)
        path.close()
    ...
    private func rectForBorder(_ bounds: CGRect) -> CGRect {
        let newRect = CGRect(x: 0, y: 0, width: bounds.size.width, height:
            bounds.size.height - font!.lineHeight + textFieldInsets.y)
